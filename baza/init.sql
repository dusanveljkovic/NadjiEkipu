...
CREATE TABLE IF NOT EXISTS `nadji_ekipu`.`interests` (
  ...
  UNIQUE INDEX `name_UNIQUE` (`name` ASC) VISIBLE,
  INDEX `fk_created_at_idx` (`created_by` ASC) VISIBLE,
  CONSTRAINT `fk_created_at`
...
CREATE TABLE IF NOT EXISTS `nadji_ekipu`.`activity_participants` (
  ...
  UNIQUE INDEX `unique` (`activity_id` ASC, `user_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `nadji_ekipu`.`messages` (
  ...
  INDEX `fk_users_idx` (`sender_id` ASC) VISIBLE,
  CONSTRAINT `fk_chats`
    FOREIGN KEY (`chat_id`)
